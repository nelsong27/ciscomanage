...
Targetspec $targets,
Optional[Targetspec] $peserver = 'pmaster.homelab.local',
String $tclfile,
String $devuser,
Enum['ios', 'nxos'] $type
){
  ...
  get_targets($targets).each | $target |  {
    case $type {
      'ios': {
        ...
        run_command("scp -i /root/.ssh/id_rsa /etc/puppetlabs/code/environments/production/modules/ciscomanage/files/${tclfile} ${devuser}@${target}:flash:${tclfile}", $peserver, "Uploading script to ${target}")
        ctrl::sleep(5)
      ...
      'nxos': {
        # copy up the tcl file using scp.   Uses sshkey
        run_command("scp -i /root/.ssh/id_rsa /etc/puppetlabs/code/environments/production/modules/ciscomanage/files/${tclfile} ${devuser}@${target}:bootflash:${tclfile}", $peserver, "Uploading script to ${target}")
        ctrl::sleep(5)
        # run command on the target to run the script that was uploaded
