  ...
  get_targets($targets).each | $target | {
    case $type {
      'ios': {
        ...
        run_command("scp -i ~/.ssh/id_rsa /Users/jerrymozes/code/cisco/config/Boltdir/site-modules/cisco/files/${tclfile} ${devuser}@${target}:flash:${tclfile}", $host, 'Uploading script to device')
        ctrl::sleep(5)
      ...
      'nxos': {
        # copy up the tcl file using scp.   Uses sshkey
        run_command("scp -i ~/.ssh/id_rsa /Users/jerrymozes/code/cisco/config/Boltdir/site-modules/cisco/files/${tclfile} ${devuser}@${target}:bootflash:${tclfile}", $host, 'Uploading script to device')
        ctrl::sleep(5)
        # run command on the target to run the script that was uploaded
