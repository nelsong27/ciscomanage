...
Puppet::Resource::ResourceType3.new(
  'tacacs_global',
  [
    # Enable or disable TACACS functionality [true|false] (a Optional[Boolean])
    # 
    ...
    # The source interface used for TACACS packets (array of strings for multiple). (a Optional[Array[String]])
    ...
    # Whether to allow users to specify tacacs server to use with `@server'. (a Optional[Boolean])
    # 
    # Valid values are `true` (also called `true, true`), `false` (also called `false, false`).
    Puppet::Resource::Param(Variant[Boolean, Enum['true', 'true', 'true', 'false', 'false', 'false']], 'directed_request')
  ],
  [
    # Resource identifier, not used to manage the device (a String)
    # 
    # Values can match `//`.
