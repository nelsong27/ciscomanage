  ...
  get_targets($targets).each | $target |  {
    case $type {
      'ios': {
        ...
        run_command("scp -i /home/jerry/.ssh/id_rsa /etc/puppetlabs/code/environments/production/modules/ciscomanage/files/${tclfile} ${devuser}@${target}:flash:${tclfile}", $peserver, "Uploading script to ${target}")
        ctrl::sleep(5)
