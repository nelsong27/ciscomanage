...
Puppet::Resource::ResourceType3.new(
  ...
  [
    ...
    # ACL Entry Protocol (a Optional[Variant[Enum["ahp","eigrp","esp","gre","icmp","igmp","ip","ipinip","nos","ospf","pcp","pim","tcp","udp"],Pattern[/\d+/]]])
  ...
  [
    ...
    # 
    # Values can match `//`.
    Puppet::Resource::Param(Pattern[//], 'access_list', true),

    # Name. Used as sequence number <1-2147483647> (a String)
    # 
    # Values can match `//`.
    Puppet::Resource::Param(Pattern[//], 'entry', true),

    # Type of access list - standard, extended, reflexive or no type (a Enum["standard","extended","reflexive","none"])
    Puppet::Resource::Param(Any, 'access_list_type', true)
  ],
  {
    /^(?<access_list>.*[^\s])\s(?<access_list_type>.*[^\s])\s(?<entry>.*)$/ => ['access_list', 'access_list_type', 'entry']
  },
  true,
  false)
